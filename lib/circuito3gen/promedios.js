...
/**
 * Calcula el bonus por jugar (ratio de participación)
 * Fórmula: PARTIDOS JUGADOS / PARTIDOS JUGADOS DE TODA LA DIVISIÓN
 * @param {number} partidosJugados - Cantidad de partidos jugados por el jugador
 * @param {number} partidosDivision - Total de partidos jugados en la división
 * @param {number} jugadoresInscriptos - Cantidad de jugadores inscriptos en la división (no usado, pero se mantiene para compatibilidad)
 * @returns {number} Bonus por jugar (ratio 0-1)
 */
export function calcularBonusPorJugar(partidosJugados, partidosDivision, jugadoresInscriptos) {
  ...
  const bonus = partidosJugados / partidosDivision
  // Redondear a 2 decimales
 ...
 * Fórmula: promedio_individual + promedio_general + bonus_por_jugar
 ...
 * @returns {number} Promedio final (suma de todos los promedios)
 */
export function calcularPromedioFinal(promedioIndividual, promedioGeneral, bonus) {
  const promedio = promedioIndividual + promedioGeneral + bonus
  // Redondear a 2 decimales
  return Math.round(promedio * 100) / 100
}
...
/**
 * Obtiene los datos completos de una división (mínimo requerido, jugadores inscriptos, partidos jugados)
 * @param {string} etapaId - ID de la etapa
 * @param {string} divisionId - ID de la división
 * @returns {Promise<Object>} Objeto con minimo_requerido, jugadores_inscriptos y partidos_jugados
 */
export async function obtenerDatosDivision(etapaId, divisionId) {
  try {
    // Usar service role para bypass RLS si está disponible, sino usar cliente normal
    const client = supabaseServiceRole || supabase
    ...
    const partidosJugados = partidosDivision || 0
    const jugadores = jugadoresInscriptos || 0
    const minimoRequerido = calcularMinimoRequerido(partidosJugados, jugadores)

    return {
      minimo_requerido: minimoRequerido,
      jugadores_inscriptos: jugadores,
      partidos_jugados: partidosJugados
    }
  } catch (error) {
    console.error('Error al obtener datos de división:', error)
    return {
      minimo_requerido: 0,
