...
/**
 * Cuenta cuántos partidos WO (no presentado) tiene un jugador.
 * @param {Array} partidos - Array de partidos
 * @param {string} usuarioId - ID del usuario
 * @returns {number} Cantidad de WO del jugador
 */
function contarWOPorJugador(partidos, usuarioId) {
  if (!partidos?.length) return 0
  return partidos.filter((p) => {
    const woIds = Array.isArray(p?.wo_jugador_ids) ? p.wo_jugador_ids : []
    return woIds.includes(usuarioId)
  }).length
}

/**
 * Excluye partidos donde el jugador tuvo WO (no se presentó).
 * Esos partidos no deben contabilizarse para estadísticas del jugador.
 * @param {Array} partidos - Array de partidos
 * @param {string} usuarioId - ID del usuario
 * @returns {Array} Partidos que sí cuentan para el jugador (sin WO para él)
 */
function partidosSinWOParaJugador(partidos, usuarioId) {
  if (!partidos?.length) return []
  const woIds = (p) => (Array.isArray(p?.wo_jugador_ids) ? p.wo_jugador_ids : [])
  return partidos.filter((p) => !woIds(p).includes(usuarioId))
}

/**
 * Calcula las estadísticas de un jugador basado en sus partidos.
 * No se contabilizan partidos donde el jugador tuvo WO (wo_jugador_ids).
 * @param {string} usuarioId - ID del usuario
...
export function calcularEstadisticasJugador(usuarioId, partidosJugador) {
  const partidosQueCuentan = partidosSinWOParaJugador(partidosJugador || [], usuarioId)
  let partidosGanados = 0
  let partidosJugados = partidosQueCuentan.length
  let diferenciaSets = 0
  let diferenciaGames = 0

  partidosQueCuentan.forEach(partido => {
 ...
 * No se cuentan partidos donde el jugador tuvo WO.
 * @param {string} usuarioId - ID del usuario
 * @param {Array} partidosJugador - Array de partidos del jugador
...
export function calcularRankingCompleto(inscripciones, partidos, partidosDivision, jugadoresInscriptos) {
  ...
  const rankings = inscripciones.map(inscripcion => {
    ...
    const wo_count = contarWOPorJugador(partidosJugador, usuarioId)
