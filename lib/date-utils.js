...
/**
 * Formatea fecha y hora de un partido en formato corto (para modales, etc.)
 * Ej: "7 feb 2026 · 20:00" en lugar de "07 de febrero de 2026 a las 08:00 p. m."
 * @param {string} fechaPartido - Fecha en formato YYYY-MM-DD
 * @param {string} horario - Hora en formato HH:MM:SS o HH:MM
 * @returns {string} - Fecha y hora en formato compacto
 */
export const formatPartidoDateTimeShort = (fechaPartido, horario) => {
  if (!fechaPartido) return ''
  const monthNames = ['ene', 'feb', 'mar', 'abr', 'may', 'jun', 'jul', 'ago', 'sep', 'oct', 'nov', 'dic']
  const [year, month, day] = fechaPartido.split('-')
  const dia = parseInt(day, 10)
  const mes = monthNames[parseInt(month, 10) - 1]
  const fechaStr = `${dia} ${mes} ${year}`
  if (!horario) return fechaStr
  const hora = horario.length >= 5 ? horario.substring(0, 5) : horario
...
/**
 * Obtiene la fecha actual en zona horaria Argentina como YYYY-MM-DD.
 * Evita el problema de toISOString() que usa UTC y puede mostrar el día incorrecto.
 * @returns {string} - Fecha en formato YYYY-MM-DD (hoy en Argentina)
 */
export const getTodayArgentina = () => {
  ...
  return formatter.format(new Date()) // en-CA da YYYY-MM-DD
}

/**
 * Parsea una fecha del servidor (YYYY-MM-DD o ISO) y devuelve YYYY-MM-DD
 * para usar en inputs type="date". Evita problemas de zona horaria.
