 ...
 * Usa la fórmula del plan: (promedio_individual * 0.7) + (promedio_general * 0.2) + bonus_por_jugar
 * @param {string} usuarioId - ID del usuario
...
export async function actualizarPromedioGlobalJugador(usuarioId) {
  try {
    ...
    let partidosGanados = 0
    let partidosJugados = todosPartidos?.length || 0
    ...
    // Para el bonus, necesitamos calcular si cumple el mínimo global
    // El mínimo global se calcula similar al mínimo por división pero considerando todo el circuito
    // Necesitamos obtener el total de jugadores únicos que han jugado en el circuito
    const { count: totalJugadoresCircuito, error: errorJugadores } = await supabase
      .from('circuitooka_partidos')
      .select('jugador_a1_id', { count: 'exact', head: true })
      .eq('estado', 'jugado')
    
    // Obtener jugadores únicos que han participado
    const { data: partidosUnicos, error: errorUnicos } = await supabase
      .from('circuitooka_partidos')
      .select('jugador_a1_id, jugador_a2_id, jugador_b1_id, jugador_b2_id')
      .eq('estado', 'jugado')
    
    if (!errorUnicos && partidosUnicos) {
      const jugadoresUnicos = new Set()
      partidosUnicos.forEach(p => {
        if (p.jugador_a1_id) jugadoresUnicos.add(p.jugador_a1_id)
        if (p.jugador_a2_id) jugadoresUnicos.add(p.jugador_a2_id)
        if (p.jugador_b1_id) jugadoresUnicos.add(p.jugador_b1_id)
        if (p.jugador_b2_id) jugadoresUnicos.add(p.jugador_b2_id)
      })
      
      // Calcular mínimo requerido global: total_partidos / (total_jugadores / 2)
      const minimoGlobalAproximado = jugadoresUnicos.size > 0
        ? Math.ceil((totalPartidosCircuito || 0) / (jugadoresUnicos.size / 2))
        : 0
      
      const bonus = partidosJugados >= minimoGlobalAproximado ? 0.1 : 0
      
      // Calcular promedio final según la fórmula del plan:
      // promedio_final = (promedio_individual * 0.7) + (promedio_general * 0.2) + bonus_por_jugar
      const promedioFinal = calcularPromedioFinal(promedioIndividual, promedioGeneral, bonus)
      
      // Guardar como decimal (0-1) para consistencia con circuitooka_rankings.promedio_final
      ...
      // Fallback si hay error al obtener jugadores únicos
      const minimoGlobalAproximado = totalPartidosCircuito > 0 
        ? Math.ceil(totalPartidosCircuito / 20) // Aproximación: 5% del total de partidos
        : 0
    ...
    // Convertir a porcentaje (0-100) para guardar en el perfil
    const promedioGlobal = promedioFinal * 100

    // Actualizar el perfil del jugador
    const { data, error } = await supabase
      .from('usuarios')
      .update({
        promedio_global: Math.round(promedioGlobal * 100) / 100, // Redondear a 2 decimales
        partidos_totales_jugados: partidosJugados,
        partidos_totales_ganados: partidosGanados,
        promedio_actualizado_at: new Date().toISOString()
      })
 ...
 * IMPORTANTE: Solo asigna posiciones a jugadores que cumplen el mínimo requerido
 * Los jugadores que no cumplen el mínimo no tienen posición (null) y no participan en ascensos/descensos
 * @param {string} etapaId - ID de la etapa
 * @param {string} divisionId - ID de la división
