...
export default function InscripcionesPage() {
      ...
      let query = supabase
        ...
        .select(`
          *,
          ...
          division_solicitada_rel:circuitooka_divisiones!circuitooka_inscripciones_division_solicitada_fkey (
            id,
            numero_division,
            nombre
          ),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = '/api/circuitooka/inscripciones'
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const session = await supabase.auth.getSession()
      const response = await fetch(`/api/circuitooka/inscripciones?id=${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${session.data.session?.access_token}`
        }
